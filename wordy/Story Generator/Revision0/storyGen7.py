...
#		self.arcState=[["friend",4],["home",2],["family",4]] # make dictionary?
		self.curState=[["home",2],["family",4],["friend",-4]] # 8 out of 10.
	def describe(self):
#		for each in self.normalEmotion
		aspectStr=[self.name]
		for aspect in self.curState:
			if aspect[1]<0: #i.e. negative
				aspectStr.extend(["doesn't have",aspect[0]])
			else:
				aspectStr.extend(["has",aspect[0]])
		story.state(aspectStr)
