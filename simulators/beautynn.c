...
//     printf("random number %d\n", random_no);
    ...
    for (int i=0;i<NO_OF_LAYERS; i++) {
        for (int j=0; j<NEURONS_PER_LAYER; j++) {
            ...
            if( i+1 == NO_OF_LAYERS ) brain[i][j].next = 0; // dodgy?
            else brain[i][j].next = & brain[i+1][rand()%NEURONS_PER_LAYER];
        }
        printf("\n");
    }
//     printf("the next of neuron %d has the power %d\n", 0, brain[0][0].next->power);
    ...
    for(int i=0;i<NEURONS_PER_LAYER;i++) random_input[i] = rand()%200;
    
    printf("\nRandom input is: "); for(int i=0;i<NEURONS_PER_LAYER;i++) printf("%d, ", random_input[i]);
    ...
    printf("\nOutput is: "); for(int i=0;i<NEURONS_PER_LAYER;i++) printf("%d, ", output[i]);
